...
export class DicerollerComponent implements OnInit {
  ...
  // D20 rolles outcomes:
  valueD20 = 20;
  valueD20Advantage: string | undefined;
  ...
  returnDamagePlusMod: number | undefined;
  arrayOfDamageDie = [] as string[];
  ...
  getDamage(numberOfDie: number, dieType: number) {
  ...
  multipleAttacks(rollsWith: string) {
    ...
    for (let index = 0; index < this.numberofAttacksHTML; index++) {
      ...
      let resultFirstD20: number | undefined;
      let resultSecondD20: number | undefined;
      let resultFinalD20: number | undefined;
      let damageDieTotal: number | undefined;

      if (rollsWith === 'advantage') {
        // tslint:disable-next-line: no-var-keyword
        [resultFirstD20, resultSecondD20, resultFinalD20] =
          this.getRoll20Advantage('advantage');
      } else if (rollsWith === 'notAdvantage') {
        // tslint:disable-next-line: no-var-keyword
        [resultFirstD20, resultSecondD20, resultFinalD20] =
          this.getRoll20Advantage('notAdvantage');
      } else if (rollsWith === 'disAdvantage') {
        // tslint:disable-next-line: no-var-keyword prefer-const
        [resultFirstD20, resultSecondD20, resultFinalD20] =
          this.getRoll20Advantage('disAdvantage');
      }

      if (resultFinalD20 === undefined) {
        // Handle the case when resultFinalD20 is not assigned
        continue;
      }

      if (resultFinalD20 === 20) {
        // Natural 20 Crit:
        const totalCritNumberofDieToRoll =
          this.dieNumberHTML * 2 + this.brutalCriticalHTML;

        // console.log(`Total of all die to roll: ${totalCritNumberofDieToRoll}`);

        // tslint:disable-next-line: no-var-keyword
        damageDieTotal = this.getDamage(
          totalCritNumberofDieToRoll,
          this.dieTypeHTML
        );
        diceTotal =
          damageDieTotal +
        ...
        // tslint:disable-next-line: no-var-keyword prefer-const
        damageDieTotal = this.getDamage(this.dieNumberHTML, this.dieTypeHTML);
        diceTotal =
          damageDieTotal +
          this.bonusDamageValueHTML +
          this.bonusRageDamageValueHTML;
      ...
      const newItem = {
        ...
        damageDie: damageDieTotal || 0, // this.getDamage(),
        ...
        damageTotal: diceTotal || 0, // this.returnDamagePlusMod,
        bless: blessValue,
        bane: baneValue,
